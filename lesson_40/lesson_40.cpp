...
и затем это 1!=1. И теперь идём обратно, то что положили последним, с него и начинаем. Возвращаем 1, и теперь 
...
{
	if (N == 0) // Один выход, если факторил вычисления введут 0.
		return 0;

	if (N == 1) //Второй выход , если факторил вычисления введут 1. (Ну и также, когда факторила введённого числа станет 1 - получим резульат вычисления).
		return 1;

	return N * Fact(N - 1); // Вычисление факториала числа.
}
//---------------------------------------------------------------------------------------------------------------------------------
void main()
{
